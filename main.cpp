...
void processFolder(const string& folderPath) {
    ...
    for (const auto& entry : fs::directory_iterator(folderPath)) {
        if (entry.is_regular_file()) {
            string title = entry.path().stem().string(); // Get the file name without extension
            titles.push_back(title);
            graph.addNode(title);
        }
    }

    // Create simple graph connections (e.g., sequential connection)
...
int main()
{
    ...
    while (true)
    {
        ...
        cout << " 10. See connection between videos \n";
        cout << " 11. Exit\n";
        cout << " Enter your choice: ";
        cin >> choice;
        cin.ignore();
